...
void NoxEngineGUI::updateAnimationPanel(NoxEngine::GameState* game_state, GUIParams* params) {
	...
	if (params->selectedEntity < 0 || params->selectedEntity + 1 > game_state->activeScene->entities.size()) {
		...
		{
			updateSequencer(ent, game_state);
		}
	}
	// Window End
	ImGui::End();
...
void NoxEngineGUI::updateSequencer(NoxEngine::Entity *ent, NoxEngine::GameState* game_state)
{
	...
	if (ImGui::CollapsingHeader("Sequencer"))
	{
		...
		mySequence.mFrameMax = animComp->numTicks[animComp->animationIndex] - 1;
		...
		if (selectedFrame < 0)
			selectedFrame = 0;
		if (selectedFrame > animComp->numTicks[animComp->animationIndex] - 1)
			selectedFrame = animComp->numTicks[animComp->animationIndex] - 1;
